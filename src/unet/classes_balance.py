...
#balance_classes = balance_classes.drop(columns='total')
print(balance_classes)
...
ax.legend()
plt.savefig(f'classes_balance_l1_{data_loading_settings["stratified"]}_seed{data_loading_settings["random_seed"]}.png')
...
plt.savefig(f'classes_balance_l1_total_{data_loading_settings["stratified"]}_seed{data_loading_settings["random_seed"]}.png')
'''for col in balance_classes.columns:
    balance_classes[col] = round(balance_classes[col]*100 / balance_classes[col].sum(), 2)
print(balance_classes)'''
# save df to csv
