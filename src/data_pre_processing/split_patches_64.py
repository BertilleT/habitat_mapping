...
def split_img_msk(img_path, msk_path):
    ...
    img_name = img_name.split('_')[:-2]
    img_name = '_'.join(img_name)
    msk_name = msk_name.split('_')[:-2]
    msk_name = '_'.join(msk_name)
    ...
    for idx, (patch_img, patch_msk) in enumerate(zip(patches_img, patches_msk)):
        #print(patch_img.shape, patch_msk.shape)
        #print(patch_img.dtype, patch_msk.dtype)
